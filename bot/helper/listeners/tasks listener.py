...
from bot import OWNER_ID, Interval, DOWNLOAD_DIR, download_dict, download_dict_lock, LOGGER, bot_name, DATABASE_URL, \
...
class MirrorLeechListener:
    ...
    def __setModeEng(self):
        ...
        mode += f" | #{'qBit' if self.isQbit else 'ytdlp' if self.isYtdlp else 'GDrive' if (self.isClone or self.isGdrive) else 'Mega' if self.isMega else 'Direct' if self.source_url and self.source_url != self.message.link else 'Tg'}"
